...
- A panel fills the opening bounded by its supports (`units_x = support length`), derived from `panel.scad`'s inter-fit deduction. Panels are configured per opening (front/back bays per row, sides per row, spans of every frame including shelves), in to-scale face drawings (front, back, sides, and a plan per frame) or by clicking the opening in the 3D view, with hover synced both ways; whole-face shortcuts remain. Panel geometry in the preview is built from the panel model's dimensions rather than loaded. Panel pins are an estimate and listed separately. (Per-opening panels added 2026-09-02; before that one panel type applied to a whole face.)
